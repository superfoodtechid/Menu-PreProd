...
4. Menampilkan pratinjau perubahan (*diff preview*) yang interaktif di dashboard frontend.
5. Mem-push pembaruan (Nama Item, Harga, Foto Link, Deskripsi, dan Nama Kategori) secara otomatis ke GoFood merchant backend.
...
* **Tabel Pratinjau Perubahan**:
  ...
  - Menampilkan visualisasi perubahan `[Nilai Lama → Nilai Baru]` dengan pencoretan teks lama.
...
* **`POST /api/jobs/push-c5`**:
  - Menerima payload `selected_sids` dan array item yang akan diperbarui.
...
* `update_menu_item`: Mengirim request `PATCH` ke `/v2/menu_groups/{group_id}/menus/{menu_id}` untuk rename kategori atau update item.
* Fallback `V1 PUT`: Mengirim request `PUT` ke `/v1/restaurants/{rest_uuid}/menu_items/{item_id}` jika V2 mengalami gangguan.
